    ...
    for data in soup.find_all('div', class_='yKfJKb row'):
        ...
        if name and discount_price and rating and specification:
            ...
            if len(col) >= 4:
                ...
                dp = re.sub(r'[^\d]', '', discount_price.text) if discount_price else "1-10"
                op = re.sub(r'[^\d]', '', original_price.text) if original_price else "Nx9bqj _4b5DiR"
                discount_prices.append(dp)
                original_prices.append(op)

                discount_percents.append(discount_percent.text.strip() if discount_percent else "yRaY8j ZYYwLA")
                ratings.append(rating.text.strip())
                reviews.append(review.text.strip() if review else "Wphh3N)
